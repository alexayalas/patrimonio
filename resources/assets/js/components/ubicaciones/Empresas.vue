<template>
    <div>
        ...
        <h1>
            Empresas
            <small>Mantenimiento de Empresas</small>
        </h1>
        <ol class="breadcrumb">
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-danger">
                        <div class="panel-heading bg-red-active"> 
                            ...
                            <div class="row">
                                <button type="button" class="btn bg-yellow pull-right mr-10" @click.prevent="LoadForm"><i class="material-icons bootstro-prev-btn mr-5">store_mall_directory</i> Nueva Empresa</button>
                            </div>                                                        
                        </div>
        ...
        <!-- PAGE CONTENT MODAL -->  
        <modal name="empresa" :width="'50%'" :height="'auto'" :scrollable="true" :clickToClose="false">
            <!-- form de registro de empresas -->
            <div class="col-md-12 modal-main pl-0 pr-0">
                <div class="row title-form">
                    <h3 class="pull-left h3-title pl-10">Registro de Empresa</h3>
                    <div class="pull-right close-form pr-20" @click="$modal.hide('empresa')"><i class="glyphicon glyphicon-remove"></i></div>                
                </div>
                <form data-sample-validation-1 class="form-horizontal form-bordered" role="form" method="POST" v-on:submit.prevent="ActionEmpresa">
                    <div class="form-body">
                        <div class="col-md-12 pt-20">
                            <div class="form-group">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" class="form-control input-sm mayusculas" name="nombre_empresa" v-model="dataEmpresa.nombre_empresa" required>
                                </div>                         
                            </div><!-- /.form-group -->                    
                            <div class="form-group">
                                <label class="col-sm-4 control-label">Dirección <span class="asterisk">*</span></label>
                                <div class="col-sm-8">
                                    <input type="text" class="form-control input-sm mayusculas" name="direccion" v-model="dataEmpresa.direccion" required>
                                </div>
                            </div><!-- /.form-group -->
                            <div class="form-group">
                                <label class="col-sm-4 control-label">RUC <span class="asterisk">*</span></label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control input-sm" name="ruc" v-model="dataEmpresa.ruc" maxlength="11" required>
                                </div>
                            </div><!-- /.form-group -->
                            <div class="form-group">
                                <label class="col-sm-4 control-label">Email </label>
                                <div class="col-sm-8">
                                    <input type="email" class="form-control input-sm" name="email" v-model="dataEmpresa.email">
                                </div>
                            </div><!-- /.form-group -->                                           
                    ...
                    <div class="col-md-12 pt-20 mt-20 mb-10 mt-0 pr-20 separator">
                        <div class="pull-right pr-10">
                            <button type="button" class="btn btn-danger active" @click="$modal.hide('empresa')"><i class="fa fa-reply-all"></i> Cancelar</button>
...
export default {
    ...
    methods: {
        ...
        createEmpresa: function(){
            ...
            toastr.success('Nueva Empresa creada con exito');
            }).catch(error => {
        ...
        updateEmpresa: function(){
            ...
            axios.put(url, this.dataEmpresa).then(response => {
                ...
                this.labelButton = 'Grabar Datos'             
                this.$modal.hide('empresa');
                toastr.success('la Empresa fue actualizada con exito');          
            }).catch(error => {
                this.errors = error.response.data.status;
                toastr.error("Hubo un error en el proceso: "+this.errors);
                this.ShowIcon = false
        ...
        processEdit(empr){
            ...
            this.dataEmpresa = {
                ...
                direccion:dataempr.direccion,
                ruc:dataempr.ruc,
                email:dataempr.email             
            }            
            this.$modal.show('empresa')
        
        },
        processDelete(id){
            this.$dialog.confirm("<span style='color:red'><strong>¿ Desea Eliminar esta Empresa ?</strong></span>", {
